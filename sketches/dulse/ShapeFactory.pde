class ShapeFactory {
  ...
  VerletShape2D createShape(VerletParticle2D center) {
    ...
    color strokeColor = getRandomStrokeColor();
    int res = (int)random(3, 8);
    float radius = random(30, 50);
    Segment[] segments = createPolySegments(center, radius, res);
    return new VerletShape2D(center, segments, col, strokeColor, anchorStrength, controlStrength);
  }
  ...
  int getRandomStrokeColor() {
   // int value = random(2);
    Boolean diff = (int)random(2) == 1;
    println(diff);
    return  diff ? color(200) : color(10);
  }
  VerletShape2D createShape(BezierShape bS) {
    color strokeColor = getRandomStrokeColor();

    color col = getRandomColor();//color(random(200, 240), random(200, 240), random(200, 240), alpha);
    Segment[] segs = createBezierShapeSegments(bS.lines);
    VerletParticle2D center = new VerletParticle2D(getCenterPoint(segs));

    return new VerletShape2D(center, segs, col, strokeColor, anchorStrength, controlStrength);
  }
